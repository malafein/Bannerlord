...
# MangoHUD per-game config written dynamically so inotify hot-reload updates the HUD in-game.
# Set to None to disable MangoHUD output.
MANGOHUD_CONF        = os.path.expanduser("~/.config/MangoHud/wine-Bannerlord.conf")
...
def _save_run_state() -> None:
    try:
        data = {
            ...
            "steps": [
                {"id": s.id, "status": s.status, "detected_logs": s.detected_logs}
...
def _load_run_state() -> bool:
    ...
    try:
        ...
        with open(STATE_FILE) as f:
            data = json.load(f)
        if data.get("plan_name") != _plan_name:
            return False
        saved = {s["id"]: s for s in data.get("steps", [])}
        for step in _state["steps"]:
            if step.id in saved:
                step.status        = Status(saved[step.id]["status"])
                step.detected_logs = saved[step.id].get("detected_logs", [])
        _state["cur_idx"] = data.get("cur_idx", 0)
        return True
    except Exception:
        return False
...
def _write_mangohud() -> None:
    ...
    try:
        ...
        if cur.status == Status.PASS:
            color  = "98c379"   # green
            status = "PASS"
        elif cur.status == Status.FAIL:
            color  = "e06c75"   # red
            status = "FAIL"
        elif cur.status == Status.SKIP:
            color  = "7a8499"   # gray
            status = "SKIP"
        elif cur.auto_pass:
            color  = "56b6c2"   # cyan — AUTO waiting
            status = "WAITING (AUTO)"
        else:
            color  = "e5c07b"   # amber — MANUAL waiting
            status = "WAITING (MANUAL)"

        line1 = f"{_plan_name}  {passed}✓ {failed}✗ {skipped}— {pending}○"
        line2 = f"Step {cur.id}/{total}: {cur.action[:46]}"

        with open(MANGOHUD_CONF, "w") as f:
            f.write(
                f"# Auto-generated by test runner — do not edit manually\n"
                f"legacy_layout=false\n"
                f"position=top-right\n"
                f"font_size=32\n"
                f"background_alpha=0.6\n"
                f"text_color={color}\n"
                f"toggle_hud=Shift_R+F12\n"
                f"custom_text={line1}\n"
                f"custom_text={line2}\n"
                f"custom_text={status}\n"
            )
    except Exception:
        pass
...
async def lifespan(app: FastAPI):
    _init_state()
    restored = _load_run_state()
    if restored:
        passed  = sum(1 for s in _state["steps"] if s.status == Status.PASS)
        failed  = sum(1 for s in _state["steps"] if s.status == Status.FAIL)
        skipped = sum(1 for s in _state["steps"] if s.status == Status.SKIP)
        print(f"[runner] Restored previous run: {passed} passed, {failed} failed, {skipped} skipped")
    _write_mangohud()
...
.btn-reset { background: transparent; border: 1px solid #3a3f55; color: var(--skip); border-radius: 5px; padding: 3px 10px; cursor: pointer; font-size: 0.78rem; }
...
<header>
  ...
  <button class="btn-reset" onclick="resetRun()">↺ Reset</button>
</header>
<main>
...
function resetRun() {
  if (!confirm('Reset all steps to pending? This cannot be undone.')) return;
